var defaultSettings = {
    ...
    walk: {
        ...
        attributeFilters: [ 'id', 'class', 'readonly', 'data-' ],
        excludeSelectors: [],
    ...
    function getStyles(elem){
        ...
        // if (elem.nodeName === 'INPUT') {console.log(ret.join(''))};
        return md5(ret.join('~'));
    }
    ...
    function getAttr(elem){
        var ret = {};
        var filters = [...elem.attributes].map(a => a.name)
        // var filters = ATTR_FILTERS.slice(0);
        var hasAttr = false;
        // if(elem.tagName.toLowerCase() === 'input'){
        //     filters.push('type');
        // }
        // filters.push(...[...elem.attributes].filter(a => /^data\-/.test(a.name)).map(a => a.name))
        filters.forEach(function(key){
